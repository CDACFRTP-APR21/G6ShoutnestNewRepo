...
def pendingRequestsApi(request,UserId=0):
    if request.method == 'GET':
        if UserId != 0:
            """ friends=Friends.objects.filter(userId=UserId) | Friends.objects.filter(friendId=UserId) """
            friends=Friends.objects.filter(friendId=UserId)
            friends_serializers = FriendsSerializer(friends,many=True)
            friend_list=friends_serializers.data
            friends_set=set()
            for i in friend_list:
...
def getSuggestions(request, UserId=0):
    if request.method == 'GET':
        if UserId != 0:
            ...
            userlist_set2=set()
            for i in userlist_set1:
                cur_user=User.objects.get(userId=UserId)
                if(cur_user!=i):
                    userlist_set2.add(i)
   
            user_serializer = UserSerializer(userlist_set2,many=True)
    ...
    if request.method == 'GET':
        if UserId != 0:
            ...
            for i in friends_set:
                ...
                userlist=Shouts.objects.filter(userId=i).values('caption','photoFileName','userId__userName','shoutId','userId','type','uploadDate','path','userId__profilePic')
                for s in userlist:
